...
{
    public class EntityMultiLocationsListQueryHandler<TUnitOfWork, TEntity, TKey>
        : DataContextHandlerBase<TUnitOfWork, EntityMultiLocationsListQuery<TEntity, ResponseArray<EntityLocationModel<TKey>>>, ResponseArray<EntityLocationModel<TKey>>>
        where TEntity : class, IMultiPointEntity, IHaveIdentifier<TKey>
        where TUnitOfWork : IUnitOfWork
    {
        private readonly IDistributedCache Cache;
        public EntityMultiLocationsListQueryHandler(ILoggerFactory loggerFactory, TUnitOfWork dataContext, IMapper mapper, IDistributedCache cache)
            : base(loggerFactory, dataContext, mapper)
        {
            Cache = cache;
        }

        protected override async Task<ResponseArray<EntityLocationModel<TKey>>> ProcessAsync(EntityMultiLocationsListQuery<TEntity, ResponseArray<EntityLocationModel<TKey>>> request, CancellationToken cancellationToken)
        {
            ...
            catch (Exception ex)
            {
                ...
                entityResponse.ReturnMessage.Add(ex.Message);
                entityResponse.ReturnMessage.Add(ex.Data.ToString());
                entityResponse.ReturnMessage.Add("Record not found");
                entityResponse.Status = false;
            }
            return entityResponse;
        }
        protected virtual IQueryable<TEntity> BuildQuery(EntityMultiLocationsListQuery<TEntity,ResponseArray<EntityLocationModel<TKey>>> request, IQueryable<TEntity> query)
        {
        ...
        protected virtual async Task<ResponseArray<EntityLocationModel<TKey>>> EfQueryList(EntityMultiLocationsListQuery<TEntity,ResponseArray<EntityLocationModel<TKey>>> request, IQueryable<TEntity> query, CancellationToken cancellationToken)
        {
            ...
            try
            {
                ...
                var items = query.Any() ? await query.Sort(request.Filter.DynamicSorting)
                    ...
                    {
                        Id = e.Id,
                        Locations = Mapper.Map<MultiPointModel>(e.Locations)
        ...
        protected virtual async Task<IReadOnlyCollection<EntityLocationModel<TKey>>> CacheQueryList(EntityMultiLocationsListQuery<TEntity,EntityLocationModel<TKey>> request, IQueryable<TEntity> query, CancellationToken cancellationToken)
        {
